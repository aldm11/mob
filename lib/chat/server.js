...
io.sockets.on("connection", function(socket){
	...
	socket.on("logout", function(account_id){
		try{
			...
			console.log("prije accounti " + sockets);
			delete sockets[account_id];
			console.log("poslije accounti " + sockets);

			
			store_client.del(["conversations", account_id].join(":"));

			// console.log("konverzacije poslije");
			// store_client.lrange(["conversations", account_id].join(":"), 0, -1, function(err, items){
				// for (var ind in items){
					// console.log("jedan xx "+ind);
				// }
			// });
	...
	function store_message(message){
		...
		store_client.lrange(["conversations", message.sender_id].join(":"), 0, -1, function(err, items){
			for(var i in items){
